...
class Database:
    ...
    def insert_user(self, name):
        self.cursor.execute("SELECT * FROM credenciais WHERE nome_usuario = 'admin'",)
        if self.cursor.fetchone() is None:
            self.cursor.execute("INSERT INTO credenciais WHERE nome_usuario = 'admin'")
            self.conn.commit()
            print("Usuário admin criado com sucesso.")
    # Novo método para verificar as credneciais de login
    def verificar_credenciais(self, nome_usuario, senha):
        self.cursor.execute("SELECT * FROM credenciais WHERE nome_usuario = ? AND senha = ?", (nome_usuario, senha))
        return self.cursor.fetchone() is not None
    ...
    def insert_user(self, nome, email, telefone):
        self.cursor.execute('INSERT INTO users (nome, email, telefone) VALUES (?, ?, ?)', (nome, email, telefone))
        self.conn.commit()
    ...
    def update_user(self, id, nome, email, telefone):
        self.cursor.execute('UPDATE users SET nome = ?, email = ?, telefone = ? WHERE id = ?', (nome, email, telefone, id))
        self.conn.commit()
...
class TelaLogin(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        self.nome_usuario_entry = ctk.CTkLabel(self.frame, placeholder_text="Nome de usuário")
        self.nome_usuario_entry.pack(pady=5, padx=10, fill="x")

        self.senha_entry = ctk.CTkLabel(self.frame, placeholder_text="Senha", show="*")
        self.senha_entry.pack(pady=5, padx=10, fill='x')

        self.login_btn = ctk.CTkEntry(self.frame, text='Entrar', command=self.fazer_login)
...
# classe pricipal para a tela de cadastro
class TelaCadastro(ctk.CTk):
    ...
    # configure a interface do usuário
    def setup_ui(self):
        ...
        # botões
        self.cadastrar_btn = ctk.CTkButton(self.btn_frame, text='Cadastrar', image=self.cadastrar_icon, compound='left', fg_color='green', hover_color='darkgreen', command=self.cadastrar)
        self.cadastrar_btn.pack(side='left', padx=5)

        self.cancelar_btn = ctk.CTkButton(self.btn_frame, text='Cancelar', image=self.cancelar_icon, compound='left', fg_color='darkred', hover_color='red', command=self.quit)
        self.cancelar_btn.pack(side='left', padx=5)

        self.listar_btn = ctk.CTkButton(self.btn_frame, text='Listar Cadastros', image=self.listar_icon, compound='left', command=self.abrir_lista)
        ...
        def limpar_campos(self):
            ...
            self.tefelone_entry.delete(0, 'end')
        ...
        def alternar_tema(self):
            if ctk.get_appearance_mode() == "Dark":
                ...
                self.team_btn.confiure(image=self.dark_icon)
...
class TelaLista(ctk.CTkToplevel):
    ...
    def setup_ui(self):
        ...
        # Carrega ícones para os botões
        self.atualizar_icon = ctk.CTkImage(Image.open(os.path.join(script_dir, "atalizar_icon.png")), size=(20,20))
        self.excluir_icon = ctk.CTkImage(Image.open(os.path.join(script_dir, "excluir_icon.png")), size=(20,20))
        self.excluir_icon = ctk.CTkImage(Image.open(os.path.join(script_dir, "voltar_icon.png")), size=(20,20))

        # Botões
        self.atualizar_btn = ctk.CTkButton(self.btn_frame, text="Atualizar", image=self.atualizar_icon, compound="left", fg_color="green", hover_color="darkgreen", command=self.atualizar_usuario)
        self.atualizar_btn.pack(side="left", padx=5)

        self.excluir_btn = ctk.CTkButton(self.btn_frame, text="Excluir", image=self.excluir.icon, compound="left", fg_color="darkred", hover_color="red", command=self.excluir_usuario)
    ...
    def configurar_estilo_treeview(self):
        ...
        if modo == "Dark":
            ...
            self.style.configure("Treeview", background= "#2a2d2e")
  ...
  # Configura o estilo da Treeview de acordo com o tema
    ...
    def atualizar_usuário(self):
        selected = self.tree.selection()
        if not selected:
            messagebox.showerror("Erro", "Por favor, selecion um usuário para atualizar.")
            return
        ...
        ctk.CTkLabel(update_window, text="Telefon:").pack()
    ...
    def excluir_usuario(self):
        selected = self.tree.selection()
        if not selected:
            messagebox.showerror("Error", "Por favor, selecione un usuario para excluir.")
            return
